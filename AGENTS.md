...
手机二维码接收的 HTTP 协议和网页由 `share/download_server.py` 管理，网卡选择留在 UI；上传使用独占创建防止并发覆盖，异常写入清理不完整文件。
手机上传/下载共用 `ui/qr_panel.py` 的网卡和二维码组件；网卡偏好保存在用户配置，修改名称或设置时必须保留该偏好。
...
- UI 风格、窗口形态、toast 行为和交互规范同步维护到 `DESIGN.md`。
